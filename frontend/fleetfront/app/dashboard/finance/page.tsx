...
export default function FinanceDashboard() {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    else console.error('Finance summary error:', res.error);
    setLoading(false);
  }, []);
            ...
            <CardContent className="pt-8">
              <form onSubmit={handleRecord} className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" required placeholder="e.g. Fuel, Maintenance, Dispatch Invoice..."
                ...
                <div className="flex gap-4 pt-4">
                  <Button type="submit" disabled={submitLoading} className="bg-[#1A1A1A] text-white rounded-none text-[10px] tracking-[0.2em] uppercase px-10 py-6">
                    {submitLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Add to Ledger'}
                  </Button>
              ...
              <Table>
                ...
                <TableBody>
                  {summary?.recentActivity?.length > 0 ? summary.recentActivity.map((t: any) => (
                    ...
                    <TableRow><TableCell colSpan={4} className="text-center py-20 text-[#C4BFAF] italic">No ledger entries posted.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
